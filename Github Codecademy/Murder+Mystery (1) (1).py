...
# define a function to return the average sentence length of a given string
# assume the average sentence length is measured is words, not letters
...
##words defined by space.
    ...
    total_count = 0
    for count in sentence_lengths:
        total_count += count
        
    #return the average length of each sentence
    return(total_count/len(sentence_lengths))
...
def prepare_text(text):
    text = text.lower()
    text = text.replace('?', '') and text.replace('.', '') and text.replace('!', '')
    return text.split()
...
def ngram_creator(text_list):
    word_1 = ""
    ngram_list = []
    for word_2 in text_list:
        if word_1 == "":
            word_1 = word_2
        else:
            ngram_list.append(word_1 + " " + word_2)
            word_1 = word_2
    return ngram_list
...
def frequency_comparison(table1, table2): 
    appearences = 0
    mutual_appearences = 0
    
    for word in table1:
        if word in table2:
            if table1[word] > table2[word]:
                appearences += table1[word]
                mutual_appearences += table2[word]
            if table1[word] < table2[word]:
                appearences += table2[word]
                mutual_appearences += table1[word]
        else:
            appearences += table1[word]
    ...
    return mutual_appearences/appearences

t1={"egg": 3, "fish": 2, 'spoon': 2, 'car': 3, 'swim': 2, 'tool': 5, 'house': 1}
t2={"ram": 4, "truck": 2, 'spoon': 4, 'car': 1, 'swim': 8, 'shoe': 6, 'hair': 2}
print ("test output = " + str(frequency_comparison(t1,t2)))
